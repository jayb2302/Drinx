    ...
    public function toggleLike($cocktailId)
{
    ...
    if ($this->likeService->userHasLikedCocktail($userId, $cocktailId)) {
        ...
        if ($this->likeService->removeLike($userId, $cocktailId)) {
            ...
            $likeCount = $this->likeService->getLikesForCocktail($cocktailId);
            echo json_encode(['success' => true, 'action' => 'unlike', 'likeCount' => $likeCount, 'message' => 'Successfully unliked the cocktail.']);
        } else {
        ...
        if ($this->likeService->addLike($userId, $cocktailId)) {
            // Fetch updated like count
            $likeCount = $this->likeService->getLikesForCocktail($cocktailId);
            echo json_encode(['success' => true, 'action' => 'like', 'likeCount' => $likeCount, 'message' => 'Successfully liked the cocktail.']);
        } else {
